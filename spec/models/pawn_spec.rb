...
RSpec.describe Pawn, type: :model do
  ...
  it 'will check if a pawn\'s move is obstructed' do
    game = FactoryBot.create(:game)
    pawn = FactoryBot.create(:pawn, x_position: 4, y_position: 6, color: :white, type: :pawn, game: game)
    FactoryBot.create(:piece, x_position: 5, y_position: 6, color: :white, type: :Queen, game: game)
    expect(pawn.valid_move?(5, 6)).to eq 'occupied'

    pawn2 = FactoryBot.create(:pawn, x_position: 5, y_position: 3, color: :black, type: :pawn, game: game)
    FactoryBot.create(:piece, x_position: 4, y_position: 3, color: :white, type: :Knight, game: game)
    expect(pawn2.valid_move?(4, 3)).to eq 'occupied'
  end
